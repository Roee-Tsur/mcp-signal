...
describe('lifecycle + teardown', () => {
  it('emits loaded (and visible) on start', async () => {
    ...
    client = createTelemetry({ ...base, adapters: [adapter] });
    await client.flush();
    const names = adapter.sent.map((e) => e.event);
    expect(names).toContain('mcp_widget_loaded');
    expect(names).toContain('mcp_widget_visible');
  });
  ...
  it('beacon-flushes on visibilitychange -> hidden', () => {
    ...
    expect(names).toContain('mcp_widget_hidden');
  });

  it('emits mcp_widget_closed exactly once on pagehide', () => {
    const adapter = fakeAdapter();
    client = createTelemetry({ ...base, adapters: [adapter] });
    dispatchPageHide();
    dispatchPageHide();
    const closed = beaconedNames(adapter).filter((n) => n === 'mcp_widget_closed');
